...
def readRelationFromTSV(name, encodings, path, annotationType):
    ...
    if annotationType != "void*":
        code += """
        annotation_{name}->push_back(utils::from_string<{annot_type}>(next));
        next = f_reader.tsv_get_next();
        """.format(name=name, annot_type=annotationType)
    code += \
